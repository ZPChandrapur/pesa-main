...
export function District() {
  ...
  const loadWorks = async () => {
    setLoading(true);
    try {
      // Load all district data by default, filter by selected district/taluka/category if provided
      ...
      if (selectedTaluka) {
        query = query.eq('taluka_name', selectedTaluka);
      }
      if (selectedCategory) {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 xl:grid-cols-4 gap-3">
        {[
          {
            icon: Building2,
            label: language === 'mr' ? 'एकूण जिल्हे' : 'Total Districts',
      ...
      <div className="bg-white rounded-3xl shadow-xl p-4 border border-gray-100">
        ...
        <div className="flex gap-0 mb-6 rounded-3xl overflow-hidden">
          ...
          >
            <TrendingUp
              className={`w-5 h-5 ${
                activeTab === 'physical' ? 'text-white' : 'text-transparent'
              }`}
              style={{
                background:
          ...
          >
            <DollarSign
              className={`w-5 h-5 ${
                activeTab === 'financial' ? 'text-white' : 'text-transparent'
              }`}
              style={{
                background:
