...
import { Building2, Filter, DollarSign, TrendingUp as TrendingUpIcon } from 'lucide-react';
...
export function GramPanchayat() {
  ...
  const [workNamesMap, setWorkNamesMap] = useState<
  ...
  // Remove the old months array and generate month-year list from data
  const getMonthYear = (dateStr: string) => {
    ...
    // If value already contains '-', assume it's in "Month-YYYY" and return as is
    if (dateStr.includes('-')) return dateStr;
    // Otherwise, fallback to just the month name
    if (months.includes(dateStr)) {
      // Try to get year from elsewhere if needed, but here just return month name
      return dateStr;
    }
    return dateStr;
  };
  // Extract unique {month-year} from allWorks for filter
  ...
  // Load all works (fetch from workProgress data source)
  ...
  const filterWorks = () => {
    ...
    if (selectedVillage) {
      filtered = filtered.filter(w => w.village_id === selectedVillage);
    }
    if (selectedCategory) {
      filtered = filtered.filter(w => w.work_category === selectedCategory);
    }
    if (selectedYear) {
      filtered = filtered.filter(w => w.year === selectedYear);
    }
    if (selectedMonth) {
      filtered = filtered.filter(w => getMonthYear(w.added_month) === selectedMonth);
    }
    setWorks(filtered);
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-purple-500 to-pink-600 rounded-3xl p-8 shadow-2xl relative overflow-hidden">
        ...
        <div className="relative z-10">
          <div className="flex items-center justify-between">
            ...
            {/* Gram Panchayat filter dropdown (with Gram Panchayat names options) */}
      ...
      {/* Original Cards below header (show counts directly as numbers) */}
