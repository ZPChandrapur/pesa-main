...
export const handleDownloadFinancialExcel = async ({
    ...
    try {
        ...
        financialWorks.forEach(work => {
            ...
            entry.set(`${category}_total_received`, parseNumeric(work.total_received_fund));
        ...
        const financeLabels =
            language === 'mr'
                ? ['मंजूर निधी', 'प्राप्त निधी', 'व्याज', 'एकूण प्राप्त', 'मागील खर्च', 'चालू खर्च', 'एकूण खर्च', 'उर्वरित निधी']
                : ['Approved', 'Received', 'Interest', 'Total Received', 'Previous Exp', 'Current Exp', 'Total Exp', 'Remaining'];
        ...
        ['A', 'B', 'C', 'D'].forEach(cat => {
            if (!selectedCategory || selectedCategory === cat) {
                ...
                for (let i = 0; i < 7; i++) headerRow1.push('');
            }
        });
        ...
        ['A', 'B', 'C', 'D'].forEach(cat => {
            if (!selectedCategory || selectedCategory === cat) {
                financeLabels.forEach(l => headerRow2.push(l));
            }
        });
            ...
            ['A', 'B', 'C', 'D'].forEach(cat => {
                if (!selectedCategory || selectedCategory === cat) {
                    const values = [
                        ...
                        entry.get(`${cat}_received`) || 0,
                        entry.get(`${cat}_interest`) || 0,
